...
import { Key, Unlock, ShieldAlert, Cpu, Terminal, Play, X, CheckCircle2, RefreshCw, HardDrive, Smartphone, MapPin, Check, Mail, Search, Send } from 'lucide-react';
...
export const BitLockerBreakerModal: React.FC<BitLockerBreakerModalProps> = ({
  ...
  const submitClientForm = () => {
    ...
    setTimeout(() => {
      ...
      const summaryMsg = `BitLocker Recovery: Klient ${clientSignature} udzielił cyfrowej zgody na odblokowanie dysku.\n- Wprowadzony klucz: ${clientKey}\n- Przejęta lokalizacja sprzętu: ${clientLocation}\n- Powiadomienie e-mail zostało przesłane na adres: ${clientEmail || 'naprawapclaptop1@gmail.com'}`;
      
      if (onSendToChat) {
         onSendToChat(summaryMsg);
      }
    }, 2000);
  };
          ...
          <div className="md:w-1/2 space-y-4">
            ...
            {flowState === 'completed' && (
              <div className="p-4 bg-emerald-900/20 border border-emerald-500/30 rounded-xl text-emerald-300 text-sm flex flex-col gap-3">
                <div className="flex items-center gap-2 font-bold"><CheckCircle2 className="w-5 h-5 text-emerald-400" /> Klient zautoryzował dostęp!</div>
                <div className="bg-slate-950 p-3 rounded border border-emerald-900 font-mono text-xs text-emerald-400 break-all space-y-1">
                  <div><strong>KLUCZ:</strong> {clientKey}</div>
                  <div><strong>LOKALIZACJA SPRZĘTU:</strong> {clientLocation}</div>
                  <div><strong>CYFROWY PODPIS:</strong> {clientSignature}</div>
                  {emailSentNotice && (
                    <div className="text-[11px] text-sky-400 border-t border-slate-800 pt-1 mt-1 flex items-center gap-1">
                      <Mail className="w-3.5 h-3.5 text-sky-400 shrink-0" />
                      <span>E-mail z potwerdzeniem zgody został wysłany!</span>
                    </div>
                  )}
                </div>
